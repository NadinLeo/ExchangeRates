...
struct IndicatorView: View {
    @Environment(\.colorScheme) var colorSchema: ColorScheme
       ...
       private var indexIndicator:Int { return rangeTime.lowerBound + Int(CGFloat(rangeTime.count - 1) * positionIndicator)}
               ...
               VStack(alignment: .leading) {
                   ZStack {
                       ...
                       ForEach (self.notHiddenLines, id: \.title ) { line in
                           ...
                           .stroke(Color(line.color!), lineWidth: 2)
                           .frame(height: geometry.size.height)
                       }
                   } // ZStack
                   Spacer()
       ...
       private func betweenValue ( yInt: [Int]) -> CGFloat {
           ...
           let doubleInd = Double(rangeTime.lowerBound) + Double(rangeTime.count - 1 ) * Double(positionIndicator)
...
struct IndicatorView_Previews: PreviewProvider {
    static var previews: some View {
          NavigationView {
                  ZStack {
                    GraphsForChart(chart: ChartData.chartsData[0], rangeTime: /*18..<40*/0..<(ChartData.chartsData[0].xTime.count - 1), lineWidth : 2)
                      IndicatorView(color: Color.secondary, chart: ChartData.chartsData[0],  rangeTime: /*18..<40*/0..<(ChartData.chartsData[0].lines[0].points.count - 1))
                  }
              }
               .colorScheme(.dark)
    }
